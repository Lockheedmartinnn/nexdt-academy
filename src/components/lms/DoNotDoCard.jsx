import React, { useState } from 'react';
import { XCircle, ChevronDown } from 'lucide-react';

export default function DoNotDoCard({ section }) {
  const [expanded, setExpanded] = useState(null);
  const items = section.items || [];

  return (
    <div className="rounded-xl overflow-hidden" style={{ background: '#111827', border: '1px solid rgba(239,68,68,0.2)' }}>
      <div className="px-5 py-4" style={{ borderBottom: '1px solid rgba(239,68,68,0.15)' }}>
        <p className="text-xs font-mono font-medium tracking-wider" style={{ color: '#EF4444' }}>
          ❌ WHAT NEXDT DOES NOT DO
        </p>
        {section.title && (
          <p className="text-base font-semibold mt-1" style={{ color: '#F9FAFB' }}>{section.title}</p>
        )}
        {section.intro && (
          <p className="text-sm mt-1" style={{ color: '#6B7280' }}>{section.intro}</p>
        )}
      </div>

      <div className="divide-y" style={{ borderColor: 'rgba(239,68,68,0.1)' }}>
        {items.map((item, i) => (
          <div key={i}>
            <button
              onClick={() => setExpanded(expanded === i ? null : i)}
              className="w-full flex items-center gap-3 px-5 py-4 text-left"
            >
              <XCircle className="w-4 h-4 flex-shrink-0" style={{ color: '#EF4444' }} />
              <p className="flex-1 text-sm font-medium" style={{ color: '#F9FAFB' }}>{item.text}</p>
              <ChevronDown
                className="w-4 h-4 flex-shrink-0 transition-transform"
                style={{ color: '#4B5563', transform: expanded === i ? 'rotate(180deg)' : 'rotate(0)' }}
              />
            </button>
            {expanded === i && (
              <div
                className="px-5 pb-4 ml-7"
                style={{ background: 'rgba(239,68,68,0.04)' }}
              >
                <p className="text-sm" style={{ color: '#94A3B8' }}>{item.detail}</p>
              </div>
            )}
          </div>
        ))}
      </div>
    </div>
  );
}